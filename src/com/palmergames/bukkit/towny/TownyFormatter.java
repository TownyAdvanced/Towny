...
public class TownyFormatter {
    ...
    public static List<String> getStatus(Resident resident, Player player) {
        ...
        out.add(replaceMessagePlaceholder(TownySettings.getLangString("registered_last_online"), registeredFormat.format(resident.getRegistered()), lastOnlineFormat.format(resident.getLastOnline())));

        // Owner of: 4 plots
        // Perm: Build = f-- Destroy = fa- Switch = fao Item = ---
        // if (resident.getTownBlocks().size() > 0) {
        out.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("owner_of_x_plots"), resident.getTownBlocks().size()));
        ...
        if (TownySettings.isUsingEconomy())
            if (TownyEconomyHandler.isActive())
                out.add(replaceMessagePlaceholder(TownySettings.getLangString("status_bank"), resident.getHoldingFormattedBalance()));
        ...
        if (resident.isJailed()) {
            out.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("jailed_in_town"), resident.getJailTown()));
        }
    ...
    public static List<String> getRanks(Town town) {
        ...
        ranklist.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("rank_list_mayor"), getFormattedName(town.getMayor())));
    ...
    public static List<String> getStatus(Town town) {
        ...
        try {
            out.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_town_board"), town.getTownBoard()));
        } catch (NullPointerException e) {
        }
        // Created Date
        Long registered = town.getRegistered();
        if (registered != 0) {
            out.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_founded"), registeredFormat.format(town.getRegistered())));
        }


        // Town Size: 0 / 16 [Bought: 0/48] [Bonus: 0] [Home: 33,44]
        try {
            out.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_town_size_part_1"), town.getTownBlocks().size(), TownySettings.getMaxTownBlocks(town)) +
                    (TownySettings.isSellingBonusBlocks() ? TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_town_size_part_2"), town.getPurchasedBlocks(), TownySettings.getMaxPurchedBlocks()) : "") +
                    (town.getBonusBlocks() > 0 ? TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_town_size_part_3"), town.getBonusBlocks()) : "") +
                    (TownySettings.getNationBonusBlocks(town) > 0 ? TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_town_size_part_4"), TownySettings.getNationBonusBlocks(town)) : "") +
        ...
        if (TownySettings.isAllowingOutposts()) {
            if (TownySettings.isOutpostsLimitedByLevels()) {
                if (town.hasOutpostSpawn())
                    if (!town.hasNation())
                        out.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_town_outposts"), town.getMaxOutpostSpawn(), town.getOutpostLimit()));
                    else {
                        int nationBonus = 0;
                        try {
                        ...
                        out.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_town_outposts"), town.getMaxOutpostSpawn(), town.getOutpostLimit()) +
                                (nationBonus > 0 ? TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_town_outposts2"), nationBonus) : "")
                        );
                    }

                else
                    out.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_town_outposts3"), town.getOutpostLimit()));
            } else if (town.hasOutpostSpawn()) {
                out.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_town_outposts4"), town.getMaxOutpostSpawn()));
            }
        }
        ...
        if (TownySettings.isUsingEconomy()) {
            if (TownyEconomyHandler.isActive()) {
                bankString = TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_bank"), town.getHoldingFormattedBalance());
                if (town.hasUpkeep())
                    bankString += TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_bank_town2"), TownySettings.getTownUpkeepCost(town));
                bankString += TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_bank_town3"), town.getTaxes()) + (town.isTaxPercentage() ? "%" : "");
            }
            out.add(bankString);
        }

        // Mayor: MrSand | Bank: 534 coins
        out.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("rank_list_mayor"), getFormattedName(town.getMayor())));
        ...
        // Nation: Azur Empire
        try {
            out.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_town_nation"), getFormattedName(town.getNation())));
        } catch (TownyException e) {
        }
        ...
        out.addAll(ChatTools.listArr(residents, TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_town_reslist"), town.getNumResidents())));
    ...
    public static List<String> getStatus(Nation nation) {
        ...
        if (registered != 0) {
            out.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_founded"), registeredFormat.format(nation.getRegistered())));
        }
        // Bank: 534 coins
        String line = "";
        if (TownySettings.isUsingEconomy())
            if (TownyEconomyHandler.isActive()) {
                line = TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_bank"), nation.getHoldingFormattedBalance());

                if (TownySettings.getNationUpkeepCost(nation) > 0)
                    line += TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_bank_town2"), TownySettings.getNationUpkeepCost(nation));
        ...
        if (nation.getNumTowns() > 0 && nation.hasCapital() && nation.getCapital().hasMayor())
            out.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_nation_king"), getFormattedName(nation.getCapital().getMayor())) +
                    TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_nation_tax"), nation.getTaxes())
            );
        ...
        out.addAll(ChatTools.listArr(getFormattedNames(nation.getTowns().toArray(new Town[0])), TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_nation_towns"), nation.getNumTowns())));
        // Allies [4]: James Nation, Carry Territory, Mason Country
        out.addAll(ChatTools.listArr(getFormattedNames(nation.getAllies().toArray(new Nation[0])), TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_nation_allies"), nation.getAllies().size())));
        // Enemies [4]: James Nation, Carry Territory, Mason Country
        out.addAll(ChatTools.listArr(getFormattedNames(nation.getEnemies().toArray(new Nation[0])), TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_nation_enemies"), nation.getEnemies().size())));
            ...
            try {
                ...
                if (TownyPerms.getResidentPerms(resident).containsKey("towny.tax_exempt")) {
                    ...
                    if (town.isTaxPercentage()) {
                        out.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_res_tax"), resident.getHoldingBalance() * town.getTaxes() / 100));
                    } else {
                        out.add(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("status_res_tax"), town.getTaxes()));
    ...
    public static String replaceMessagePlaceholder(String message, Object... args) {
