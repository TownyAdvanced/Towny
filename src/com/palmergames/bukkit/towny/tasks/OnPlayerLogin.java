		...
		if (!universe.getDataSource().hasResident(player.getName())) {
			...
			try {
				...
				if (TownySettings.isShowingRegistrationMessage())				
					TownyMessaging.sendMessage(player, String.format(Translation.of("msg_registration"), player.getName()));
					...
					if (town.hasUpkeep()) {
						...
						try {
							if ((upkeep > 0) && (!town.getAccount().canPayFromHoldings(upkeep))) {
								 ...
								 */
								TownyMessaging.sendMessage(resident, String.format(Translation.of("msg_warning_delete"), town.getName()));
							}
					...
					if (town.hasNation()) {
						...
						try {
							if ((upkeep > 0) && (!nation.getAccount().canPayFromHoldings(upkeep))) {
								 ...
								 */
								TownyMessaging.sendMessage(resident, String.format(Translation.of("msg_warning_delete"), nation.getName()));
							}
						} catch (EconomyException ex) {
							// Economy error, so ignore it and try to continue.
						}
