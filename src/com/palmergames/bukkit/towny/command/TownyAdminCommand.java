...
public class TownyAdminCommand extends BaseCommand implements CommandExecutor {
			...
			case "town":
				if (args.length == 2) {
					...
					switch (args[2].toLowerCase()) {
							...
							switch (args.length) {
								...
								case 6:
									switch (args[3].toLowerCase()) {
										...
										case "remove":
											try {
												return NameUtil.filterByStart(TownyUniverse.getInstance().getResident(args[4]).getTownRanks(), args[5]);
						...
						case "set":
							try {
								return TownCommand.townSetTabComplete(TownyUniverse.getInstance().getTown(args[1]), StringMgmt.remArgs(args, 2));
						...
						case "outlaw":
							...
							case 5:
								switch (args[3].toLowerCase()) {
									...
									case "remove":
										try {
											return NameUtil.filterByStart(NameUtil.getNames(TownyUniverse.getInstance().getTown(args[1]).getOutlaws()), args[4]);
			...
			case "nation":
				if (args.length == 2) {
					...
					switch (args[2].toLowerCase()) {
						...
						case "set":
							try {
								return NationCommand.nationSetTabComplete(TownyUniverse.getInstance().getNation(args[1]), StringMgmt.remArgs(args, 2));
							} catch (NotRegisteredException e) {
								return Collections.emptyList();
							}
						case "merge":
	...
	private void parseAdminDatabaseCommand(String[] split) {
		...
		if (split.length == 0 || split.length > 2 || split[0].equalsIgnoreCase("?")) {
			...
			sender.sendMessage(ChatTools.formatCommand(Translation.of("admin_sing"), "/townyadmin database", "getString", ""));
			sender.sendMessage(ChatTools.formatCommand(Translation.of("admin_sing"), "/townyadmin database", "loadString", ""));
			return;
		}
		...
		if (split[0].equalsIgnoreCase("getString")) {
			...
			TownyUniverse.getInstance().getDatabaseHandler().loadAll();
	...
	private void parseAdminCheckPermCommand(String[] split) throws TownyException {
		...
		Player player = TownyAPI.getInstance().getPlayer(TownyUniverse.getInstance().getResident(split[0]));
		if (player == null) {
	...
	private void giveBonus(String[] split) throws TownyException {
		...
		try {
			...
			try {
				...
				town = townyUniverse.getResident(split[0]).getTown();
			}
			try {
	...
	public void parseAdminResidentCommand(String[] split) throws TownyException {
		...
		try	{
			Resident resident = townyUniverse.getResident(split[0]);
	...
	public void parseAdminTownCommand(String[] split) throws TownyException {
		...
		try {
			...
			if (split[1].equalsIgnoreCase("invite")) {
				...
				Resident resident;
				try {
					resident = townyUniverse.getResident(split[2]);
				} catch (NotRegisteredException e) {
	...
	public void parseAdminNationCommand(String[] split) throws TownyException {
		...
		try {
			...
			if (split[1].equalsIgnoreCase("add")) {
				...
				try {
					remainingNation = townyUniverse.getNation(split[2]);
				} catch (NotRegisteredException e) {
			...
			if (split.length < 3) {
				...
				try {
					...
					if (split[2].equalsIgnoreCase("npc")) {
						...
						newMayor = townyUniverse.getResident(split[2]);
					}
