...
public class Town extends TownBlockOwner implements ResidentList, TownyInviteReceiver, TownyInviteSender {
    ...
    public void addResidentCheck(Resident resident) throws AlreadyRegisteredException {
        ...
        if (hasResident(resident))
            throw new AlreadyRegisteredException(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("msg_err_already_in_town"), resident.getName(), getFormattedName()));
        else if (resident.hasTown())
            try {
                if (!resident.getTown().equals(this))
                    throw new AlreadyRegisteredException(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("msg_err_already_in_town"), resident.getName(), resident.getTown().getFormattedName()));
        ...
        if (this.hasNation() && TownySettings.getNationRequiresProximity() > 0)
            if (!this.getNation().getCapital().equals(this)) {
                ...
                if (nation.getCapital().getHomeBlock().getWorld().getName() != this.getHomeBlock().getWorld().getName()) {
                    TownyMessaging.sendNationMessagePrefixed(nation, TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("msg_nation_town_moved_their_homeblock_too_far"), this.getName()));
                    try {
        ...
        if (TownySettings.isUsingEconomy()) {
            ...
            if (bankcap > 0) {
                if (amount + this.getHoldingBalance() > bankcap) {
                    TownyMessaging.sendTownMessage(this, TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("msg_err_deposit_capped"), bankcap));
                    return;
                }
            }
    ...
    public void newReceivedInvite(Invite invite) throws TooManyInvitesException {
        if (receivedinvites.size() <= (InviteHandler.getReceivedInvitesMaxAmount(this) - 1)) { // We only want 10 Invites, for towns, later we can make this number configurable
            ...
            throw new TooManyInvitesException(TownyFormatter.replaceMessagePlaceholder(TownySettings.getLangString("msg_err_town_has_too_many_invites"), this.getName()));
        }
    }
