...
public enum TownSpawnLevel {
	...
	public boolean hasPermissionNode(Towny plugin, Player player, Town town) {
		...
		return this == TownSpawnLevel.ADMIN || ((plugin.isPermissions() && TownyUniverse.getInstance().getPermissionSource().has(player, this.permissionNode)) || ((!plugin.isPermissions()) && (isAllowedTown(town))));
	}
