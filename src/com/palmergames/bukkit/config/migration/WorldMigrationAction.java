...
public enum WorldMigrationAction {
	UPDATE_WORLD_BLOCK_IGNORE(((townyWorld, change) -> {
		if (change.type == MigrationType.REPLACE)
			townyWorld.setPlotManagementIgnoreIds(replaceAll(townyWorld.getPlotManagementIgnoreIds(), change.key, change.value));
		else
	...
	UPDATE_WORLD_DELETE_MAYOR(((townyWorld, change) -> {
		if (change.type == MigrationType.REPLACE)
			townyWorld.setPlotManagementMayorDelete(replaceAll(townyWorld.getPlotManagementMayorDelete(), change.key, change.value));
		else
	...
	UPDATE_WORLD_UNCLAIM_DELETE(((townyWorld, change) -> {
		if (change.type == MigrationType.REPLACE)
			townyWorld.setPlotManagementDeleteIds(replaceAll(townyWorld.getPlotManagementDeleteIds(), change.key, change.value));
		else
	...
	UPDATE_WORLD_EXPLOSION_REVERT_BLOCKS(((townyWorld, change) -> {
		if (change.type == MigrationType.REPLACE)
			townyWorld.setPlotManagementWildRevertMaterials(replaceAll(townyWorld.getPlotManagementWildRevertBlocks(), change.key, change.value));
		else
	...
	UPDATE_WORLD_EXPLOSION_REVERT_ENTITIES(((townyWorld, change) -> {
		if (change.type == MigrationType.REPLACE)
			townyWorld.setPlotManagementWildRevertEntities(replaceAll(townyWorld.getPlotManagementWildRevertEntities(), change.key, change.value));
		else
