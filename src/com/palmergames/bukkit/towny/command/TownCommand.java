...
public class TownCommand extends BaseCommand implements CommandExecutor, TabCompleter {
	...
	public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
		...
		if (args.length == 1) {
			return NameUtil.filterByStartOrGetTownyStartingWith(townTabCompletes, args[0], "t");
		}
		
		if (args.length == 2) {
			switch (args[0].toLowerCase()) {
				case "spawn":
					return NameUtil.getTownyStartingWith(args[1], "t");
				case "set":
		...
		if (args.length == 3) {
			switch (args[1].toLowerCase()) {
				case "remove":
				case "add":
					return NameUtil.getTownyStartingWith(args[2], "r"); // Not sure why this was set to nation previously, it should only be residents of player's town
				case "perm":
