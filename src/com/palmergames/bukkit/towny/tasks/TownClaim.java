	...
	public void run() {
		...
		if (selection != null) {
			...
			if (!claim && TownySettings.getClaimRefundPrice() > 0.0) {
				try {
					town.getAccount().add(TownySettings.getClaimRefundPrice()*selection.size(), "Town Unclaim Refund");
			...
			Confirmation confirmation = new Confirmation(() -> { 
				...
				if (TownySettings.getClaimRefundPrice() > 0.0) {
					try {
						town.getAccount().add(TownySettings.getClaimRefundPrice()*townSize, "Town Unclaim Refund");
