		...
		switch (args.length) {
			...
			case 2:
				switch (args[0].toLowerCase()) {
					...
					case "deny":
						try {
							return NameUtil.filterByStart(TownyUniverse.getInstance().getResident(sender.getName()).getReceivedInvites()
								.stream()
	...
	public static void sendInviteList(Player player, List<Invite> list, int page, boolean fromSender) {
		...
		for (int i = (page - 1) * 10; i < iMax; i++) {
			...
			if (name == null) {
				...
				try {
					name = TownyUniverse.getInstance().getResident(name).getName();
				} catch (NotRegisteredException e) {
					name = "Unknown";
				}
			}
