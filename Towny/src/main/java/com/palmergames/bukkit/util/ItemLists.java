...
public class ItemLists extends AbstractRegistryList<Material> {	
	 ...
	 */
	public static final ItemLists UNSTRIPPED_WOOD = newBuilder().endsWith("_LOG").notStartsWith("STRIPPED_").add("CRIMSON_STEM", "WARPED_STEM").add("BAMBOO_BLOCK").build();
	...
	public static Set<Material> getGrouping(String groupName) {
		...
		try {
			return ((ItemLists) ItemLists.class.getField(groupName).get(null)).tagged;
		} catch (Exception e) {
			return ImmutableSet.of();
		}
	}
