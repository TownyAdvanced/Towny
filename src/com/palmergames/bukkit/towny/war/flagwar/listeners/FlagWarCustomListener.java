...
public class FlagWarCustomListener implements Listener {
	...
	public void onCellDefendedEvent(CellDefendedEvent event) {
		...
		plugin.getServer().broadcastMessage(String.format(Translation.of("msg_enemy_war_area_defended"), playerName, cell.getCellString()));
			...
			try {
				...
				if (defendingPlayer == null) {
					...
					if (attackingPlayer.getAccount().payTo(FlagWarConfig.getDefendedAttackReward(), defendingPlayer, "War - Attack Was Defended")) {
						try {
							TownyMessaging.sendResidentMessage(attackingPlayer, String.format(Translation.of("msg_enemy_war_area_defended_attacker"), defendingPlayer.getFormattedName(), formattedMoney));
						} catch (TownyException ignored) {
						}
						try {
							TownyMessaging.sendResidentMessage(defendingPlayer, String.format(Translation.of("msg_enemy_war_area_defended_defender"), attackingPlayer.getFormattedName(), formattedMoney));
						} catch (TownyException ignored) {
						}
					}
				}
	...
	public void onCellWonEvent(CellWonEvent event) {
		...
		try {
			...
			if (TownySettings.isUsingEconomy()) {
				try {
					...
					if (amount > 0) {
						...
						// Message
						moneyTranserMsg = String.format(Translation.of("msg_enemy_war_area_won_pillage"), attackingResident.getFormattedName(), TownyEconomyHandler.getFormattedBalance(amount), defendingTown.getFormattedName());
					} else if (amount < 0) {
						...
						if (!attackingResident.getAccount().payTo(amount, defendingTown, reason)) {
							...
							TownyMessaging.sendGlobalMessage(String.format(Translation.of("msg_enemy_war_area_won"), attackingResident.getFormattedName(), (attackingNation.hasTag() ? attackingNation.getTag() : attackingNation.getFormattedName()), cell.getCellString()));
						}

						// Message
						moneyTranserMsg = String.format(Translation.of("msg_enemy_war_area_won_rebuilding"), attackingResident.getFormattedName(), TownyEconomyHandler.getFormattedBalance(amount), defendingTown.getFormattedName());
					}
			...
			if (FlagWarConfig.isFlaggedTownblockTransfered()) {
				...
				TownyMessaging.sendPrefixedTownMessage(attackingTown, String.format(Translation.of("msg_war_defender_keeps_claims")));
				TownyMessaging.sendPrefixedTownMessage(defendingTown, String.format(Translation.of("msg_war_defender_keeps_claims")));
			}
