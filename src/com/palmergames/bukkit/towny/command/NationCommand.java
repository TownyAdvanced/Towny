...
public class NationCommand extends BaseCommand implements CommandExecutor {
			...
			if (split.length == 0)
				...
				try {
					if (split.length == 1) {
						...
						nation = townyUniverse.getDatabaseHandler().getNation(split[1]);
					}
				} catch (Exception e) {
				...
				if (split[0].equalsIgnoreCase("rank")) {
					...
					try {
						final Nation nation = townyUniverse.getDatabaseHandler().getNation(split[0]);
						Resident resident = townyUniverse.getResident(player.getName());
	...
	private void parseNationJoin(Player player, String[] args) {
		...
		try {
			...
			nation = townyUniverse.getDatabaseHandler().getNation(nationName);
	...
	public void mergeNation(Player player, String name) throws TownyException {
		...
		try {
			nation = universe.getDatabaseHandler().getNation(name);
    ...
    public static void nationSpawn(Player player, String[] split, boolean ignoreWarning) throws TownyException {
        ...
        try {
            ...
            if (split.length == 0) {
                ...
                // split.length > 1
                nation = townyUniverse.getDatabaseHandler().getNation(split[0]);
