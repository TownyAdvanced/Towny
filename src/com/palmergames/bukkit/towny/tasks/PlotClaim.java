			...
			for (WorldCoord worldCoord : selection) {
				...
				try {
					...
					if (tb.hasPlotObjectGroup()) {
						final PlotGroup group = tb.getPlotObjectGroup();
						...
						if (TownyEconomyHandler.isActive() && group.getPrice() > 0) {
							...
							if (group.getResident() != null) {
								 ...
								 */
								double bankcap = group.getTown().getBankCap();
								if (bankcap > 0 && group.getPrice() + group.getTown().getAccount().getHoldingBalance() > bankcap)
						...
						if (residentGroupClaim(selection)) {
							...
							group.setPrice(-1);
							TownyMessaging.sendPrefixedTownMessage(group.getTown(), Translatable.of("msg_player_successfully_bought_group_x", player.getName(), group.getName()));
