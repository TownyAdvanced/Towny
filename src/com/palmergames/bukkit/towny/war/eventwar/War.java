...
public class War {
	...
	private void remove(Town attacker, TownBlock townBlock) throws NotRegisteredException {
		...
		try {
			...
			if (!townBlock.getTown().payTo(TownySettings.getWartimeTownBlockLossPrice(), attacker, "War - TownBlock Loss")) {
				...
				TownyMessaging.sendTownMessage(townBlock.getTown(), TownySettings.getLangString("msg_war_town_removed_from_war_titlemsg"));
	...
	private void remove(Nation nation) {
		...
		TownyMessaging.sendNationMessage(nation, TownySettings.getLangString("msg_war_nation_removed_from_war_titlemsg"));
		for (Town town : nation.getTowns())
			remove(town);
		checkEnd();
	}
